...
def wpi(request):
    ...
    WPI_ini = pd.read_excel("graphgen_app/files/monthly_index_202304.xls", sheet_name=0)
...
def exchange(request):
    ...
    exrate = pd.read_excel("graphgen_app/files/HBS_Table_No._215_ _Daily_Exchange_Rate_of_the_Indian_Rupee.xlsx", sheet_name=0)
...
def foreignreserves(request):
    ...
    forex_ini = pd.read_excel("graphgen_app/files/RBIB_Table_No._32_ _Foreign_Exchange_Reserves.xlsx", sheet_name=0)
...
def keyrates(request):
    ...
    keyrate = pd.read_excel("graphgen_app/files/HBS_Table_No._44_ _Major_Monetary_Policy_Measures_-_Bank_Rate,_CRR_&_SLR.xlsx", sheet_name=0)
    ...
    return render(request, 'key-rates.html', {'graph_div10': graph_div10,})

def gdp(request):
    #Code for 11th graph - GDP : constant
    GDP = pd.read_excel("graphgen_app/files/HBS_Table_No._156_ _Quarterly_Estimates_of_Gross_Domestic_Product_at_Market_Prices_(at_Constant_Prices)_(New_Series)_(Base _2004-05).xlsx", sheet_name=2)
    GDP = GDP.iloc[3:]
    GDP = GDP.iloc[:, 1:]
    ...
    GDP = pd.read_excel("graphgen_app/files/HBS_Table_No._154_ _Quarterly_Estimates_of_Gross_Domestic_Product_at_Market_Prices_(at_Current_Prices)_(New_Series)_(Base _2004-05).xlsx", sheet_name=2)
    GDP = GDP.iloc[3:]
    GDP = GDP.iloc[:, 1:]
    GDP.columns = GDP.iloc[0]
    GDP['Industry/ Year'] = GDP['Industry/ Year'].fillna(method='ffill')
...
def scbfoodbank(request):
    ...
    scb = pd.read_excel("graphgen_app/files/Bank_Credit_and_Food_Credit_-_Scheduled_Commercial_Banks.xlsx", sheet_name=0)
    scb = scb.iloc[6:, 4:]
...
def balanceofpayments(request):
    ...
    BoP_ini = pd.read_excel("graphgen_app/files/HBS_Table_No._194_ _Indias_Overall_Balance_of_Payments_-_Quarterly_-_Rupees.xlsx", sheet_name=1)
    ...
    return render(request, 'balance-of-payments.html', {'graph_div14': graph_div14,})

def iip(request):
    IIP_ini = pd.read_excel("graphgen_app/files/Index_of_Industrial_Production_(Detailed).xlsx", sheet_name=1)
...
def fdi(request):
    FDI_ini = pd.read_excel("graphgen_app/files/RBIB_Table_No._34_ _Foreign_Investment_Inflows.xlsx", sheet_name=1)
    #new code
...
def ggni(request):    
    GGNI_ini = pd.read_excel("graphgen_app/files/API_NY.GNP.ATLS.CD_DS2_en_excel_v2_5455179.xls", sheet_name=0)
